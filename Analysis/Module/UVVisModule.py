...
def AbsorbanceTest(data_dict:dict):
    '''
    Anaylsis multi-Peak properties
    current_func_name
    ...
    :param hyperparameter_dict (dict) : 
    ...
    '''
    raw_data=data_dict["abs"]
    reference_dict=data_dict["ref"]
    result = OrderedDict()
    
    clean_dict=getCleanSpecturm(raw_data, reference_dict)
    rawSpectrum = getSpectrumArray(clean_dict)
    sliceSpectrum = getSliceSpectrum(rawSpectrum, min=300, max=800)
    smooth_result = smooth_Boxcar(sliceSpectrum,box_size=10)
    lambdamax_list, Intensity_peaks_list, FWHM_peaks_list = analysisMultiPeak(rawSpectrum=smooth_result, prominence=0.01, width=20)
    lambdamax_single, Intensity_peaks_single, FWHM_peaks_single = analysisSinglePeak(rawSpectrum=smooth_result, prominence=0.01, width=20)

    result["lambdamax"] = lambdamax_single
    result["intensity"] = Intensity_peaks_single
    result["FWHM"] = FWHM_peaks_single

    clean_dict["Property"]=result
