...
class TCP_Class(ParameterTCP):
    ...
    def callServer_BATCH(self, command_str="info/BATCH/None/all/virtual"):
        """
        receive command_byte & send tcp packet using socket 
        1) INFO
        :param command_str="info/BATCH/None/all/{mode_type}" (str) : input command string type
            - {jobID} (str): info, get self.{}_info
            - device_name="BATCH" (str) (LA, ABS, STIRRER, PUMP, STORAGE, PIPETTE)
            - action_type=None (str) ... (str): get self.{}_info
            - action_info=all (str) ... (str)
            - mode_type = "virtual" (str): set mode type --> real, virtual
        :return each_info_dict (dict): has BATCH's device information
        ...
        if action type == "home" --> '1/LA/home/null/{mode_type}'
            ...
            - action_info="null" (str): set action info
            - mode_type = "virtual" (str): set mode type --> real, virtual 
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((self.routing_table["BATCH"]["HOST"], self.routing_table["BATCH"]["PORT"]))
            time.sleep(1)
    ...
    def callServer_UV(self, command_str="info/UV/None/all/virtual"):
        """
        receive command_byte & send tcp packet using socket 
        ...
        command_bytes=str.encode(command_str)
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((self.routing_table["UV"]["HOST"], self.routing_table["UV"]["PORT"]))
            time.sleep(1)
            s.sendall(command_bytes)
            message_recv = b''
            while True:
    ...
    def callServer_AMR(self, command_str="info/AMR/None/all/virtual"):
        """
        receive command_byte & send tcp packet using socket 
        ...
        command_bytes=str.encode(command_str)
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((self.routing_table["AMR"]["HOST"], self.routing_table["AMR"]["PORT"]))
            time.sleep(1)
            s.sendall(command_bytes)
            message_recv = b''
            while True:
...
if __name__ == '__main__':
    ...
    """
    # print(tcp_object.callServer_BATCH())
    print(tcp_object.callServer_AMR())
